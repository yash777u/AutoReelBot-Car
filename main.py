...
def generate_simple_thumbnail(video_path):
    """Generate a simple thumbnail from the first frame of the video."""
    import cv2
...
def upload_reel(video_path, caption):
    ...
    try:
        ...
        print(
            "💡 Get fresh INSTA_SESSIONID from browser cookies (see FIX_INSTAGRAM_LOGIN.md)"
        )
        return False

    print(f"📤 Uploading reel to Instagram: {os.path.basename(video_path)}")
    ...
    try:
        ...
        if success:
            try:
                ...
                print(f"🗑️ Deleted uploaded video: {selected_video}")
